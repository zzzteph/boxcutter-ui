...
TOOLS = {
    ...
    "browser-crawl": "Render a JS/SPA in a headless browser and capture its routes + API (XHR/fetch) calls.",
...
WORKFLOWS = {
    ...
    "env-nuclei": "subfinder -> nuclei across the env (takeover on all hosts; web-nuclei template passes on live hosts)",
    ...
    "env-takeover": "subfinder, then nuclei takeover templates on every discovered subdomain",
    "env-wayback-secrets": "subfinder, then secrets-scan (web-crawl + wayback -> scan-secrets) on every subdomain",
    "env-wayback": "subfinder, wayback every subdomain, then endpoint-scan every parameterised URL (deduped)",
    ...
    "recon": "subdomains (subfinder + wayback), kept if they resolve (dnsx)",
    ...
    "web-crawl": "crawl a URL with Katana + ZAP, merged and deduped - the crawl you usually want",
...
AGENTS = {
    "irvin": "Pipeline bug-hunter: suggester council -> concluder -> planner -> executors -> reporter (LLM-driven).",
